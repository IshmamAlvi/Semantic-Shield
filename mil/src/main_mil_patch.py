...
def train_patch_mil(model, device, train_dataloader, test_dataloader, optimizer, batch_size, loss_img, loss_txt, scheduler, lst_tokens, args):
    best_te_loss = 1e5
    best_ep = -1
    train_losses = []
    test_losses = []
    ...
    for epoch in range(args.epoch):
        ...
        plt.plot(steps, mil_step_losses, label='MIL patch first {} iteration Train Loss'.format(step))
        plt.xlabel('Steps')
        plt.ylabel('Loss')
        plt.title('Epoch vs Steps Curve')
        plt.xticks(np.arange(1, step, 1))
        ...
        # Display the plot
        plt.legend(loc='best')
        plt.savefig('../../../KG_Defence/mil/figures/loss_clipapi_mil_patch_steps.png')
...
if __name__ == '__main__':
    ...
    if (args.clip_openai == 'yes'):
        ...
        elif (args.baseline == 'baseline_mil_patch'):
            for param in model.parameters():
                param.requires_grad = False
            train_patch_mil(model, device, train_dataloader, test_dataloader, optimizer, batch_size, loss_img, loss_txt, scheduler, lst_tokens, args)
